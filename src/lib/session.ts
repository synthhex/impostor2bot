...
export default class Session {
	...
	public userJoined(user: GuildMember) {
		if (this.muted && !user.voice.serverMute) user.voice.setMute(true, 'In discussion phase.');
		if (!this.muted && user.voice.serverMute) user.voice.setMute(false, 'Not in discussion phase.');
	}

	public userLeft(user: GuildMember, switched: boolean = false) {
		if (this.muted && user.voice.serverMute)
			if (switched)
				// if the user switched channels, let the other sessions handle it
				user.voice.setMute(false, 'Left session.');
			else this.client.free(user); // (remember to) unmute the user if they left the session

		if (user.id === this.user.id) this.client.sessionManager.deleteSession(this.voiceChannel);
	}
	...
	public async mute() {
		...
		console.log(
			...
			this.voiceChannel.members.map((member) => member.displayName),
		);
