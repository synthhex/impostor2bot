...
export async function execute(oldState: VoiceState, newState: VoiceState) {
    ...
    if (oldState.channel === null && newState.channel !== null) {
        ...
        if (oldState.channel.type === ChannelType.GuildVoice) { // if the old channel is a voice channel
            const session = client.sessionManager.getSession(oldState.channel);
            if (session)
                session.userLeft(oldState.member!, newState.channel.type === ChannelType.GuildVoice && client.sessionManager.getSession(newState.channel)?.muted === false); // let the session handle the user leaving
        }

        if (newState.channel.type === ChannelType.GuildVoice) { // if the new channel is a voice channel
            const session = client.sessionManager.getSession(newState.channel);
            if (session)
                session.userJoined(newState.member!); // let the session handle the user joining
        }
