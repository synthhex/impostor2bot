...
async function main() {
    const commands = Array.from((await ImpostorClient.collectCommands()).values()).map((command) => command.data.toJSON());

    const rest = new REST().setToken(tryReadEnv('DISCORD_TOKEN', String));

    // Load program arguments
    const guildId = process.argv.slice(2).includes('--all') 
        ? 'A' 
        : (process.env.TEST_GUILD_ID ?? null);

    if (!guildId) {
        log("No TEST_GUILD_ID environmental variable set. If you want to refresh all guilds, run this script with the --all flag.");
        return;
    } else {
